...
| oci://ghcr.io/prometheus-community/charts | kube-state-metrics | 8.4.1 |
...
| agent.resourceMetrics.kubeStateMetrics.url | string | `""` | Existing kube-state-metrics Prometheus endpoint; the bundled Service is selected automatically when its subchart is enabled. |
| agent.resourceMetrics.legacy.enabled | bool | `true` | Emit the legacy metrics.k8s.io-backed container resource gauges during the migration release. |
| kube-state-metrics.enabled | bool | `false` | Install the minimal bundled kube-state-metrics instance. |
