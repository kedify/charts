...
| `operator.kubernetesApiTimeout` | string | `"5s"` | Additional timeout beyond the ScaledObject or ScaledJob polling interval for Kubernetes API operations in scaling loops. Setting `0s` uses the polling interval as the timeout; the timeout cannot be disabled |
...
| `metricsServer.enabled` | bool | `true` | Enable KEDA metrics server. |
...
| `metricsServer.registerAPIService` | bool | `true` | Enable registering KEDA metrics server as external metrics API resource. |
